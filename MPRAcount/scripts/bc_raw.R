...
cond_data <- read.delim(cond.data, row.names=1, header=F, stringsAsFactors=F, check.names=FALSE)
colnames(cond_data) <- "condition"

count_table <- read.delim(count.table, stringsAsFactors=F, header=T, check.names=FALSE)
